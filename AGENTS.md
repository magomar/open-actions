...
- This is a monorepo: each action is a self-contained plugin under `actions/<name>/`, with its own spec in `specs/`. New actions follow the layout and conventions in `MISSION.md`.
...
2. **Comprehensive Preflight Diagnostics:** Run `keel doctor` to diagnose environment, issue tracker, and OKF compliance; run `keel validate` to verify spec integrity and acceptance criteria before requesting approval on changes.
...
- **Walkthroughs**: Before completing a task, provide a verification receipt (walkthrough) showing evidence of test coverage and validation results.
