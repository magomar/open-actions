...
- `[x]` **[Antigravity Usage](../005_antigravity_usage.md)** (`actions/antigravity-usage/`): Display Google Antigravity compute quota (Gemini and Claude pools, prompt/flow credits, reset countdowns) on a device button — fixed metric or cycling (manual/periodic).
...
- `[x]` **[Pomodoro Timer](../006_pomodoro_timer.md)** (`actions/pomodoro-timer/`): A Pomodoro technique workflow inspired by Pomotroid with custom SVG ring gauges, round tracking, and sound alerts.
